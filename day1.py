...
On each line, the calibration value can be found by combining the first digit
 and the last digit (in that order) to form a single two-digit number. It looks
 like some of the digits are actually spelled out with letters: one, two, three,
 four, five, six, seven, eight, and nine also count as valid "digits". What is
...
def parse(input_: str) -> int:
    """
    Concatenate the first and last number in each line, then sum them all up
    """
    sum_ = 0
    for line in input_.split("\n"):
        ...
        if len(numbers) == 0:
            continue
        sum_ += int(
            force_digits(numbers[0]) + force_digits(numbers[-1])
        )
    return sum_
