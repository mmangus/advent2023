...
Each game is listed with its ID number (like the 11 in Game 11: ...) followed
 ...
 bag (like 3 red, 5 green, 4 blue). The Elf would first like to know which
 games would have been possible if the bag contained only 12 red cubes, 13
...
CubeCountPattern = re.compile(r"(\d+) (blue|green|red)")


def roll_is_possible(roll: str) -> bool:
    for count, color in CubeCountPattern.findall(roll):
        if int(count) > MAX_COUNT[color]:
            return False
    return True


def sum_valid_ids(input_: str) -> int:
    sum_ = 0
    for line in input_.split("\n"):
        game, rolls = line.split(": ")
        game_id = int(game.split(" ")[1])
        if all(roll_is_possible(roll) for roll in rolls.split("; ")):
            sum_ += game_id
    return sum_


def minimum_set_power(rolls: str) -> int:
    """
    Given a series of rolls, return the "power" of the minimum number of cubes
     required for all the rolls to be valid
    """
    minima = {"red": 0, "green": 0, "blue": 0}
    for roll in rolls.split("; "):
        for count, color in CubeCountPattern.findall(roll):
            minima[color] = max(minima[color], int(count))
    return reduce(mul, minima.values(), 1)


def sum_set_power(input_: str) -> int:
    sum_ = 0
    for line in input_.split("\n"):
        game, rolls = line.split(": ")
        sum_ += minimum_set_power(rolls)
    return sum_


INPUT = """Game 1: 19 blue, 12 red; 19 blue, 2 green, 1 red; 13 red, 11 blue
...
if __name__ == "__main__":
    print(sum_valid_ids(INPUT))
    print(sum_set_power(INPUT))
